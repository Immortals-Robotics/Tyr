                     ...
                     float near_dis_override)
{
    ...
    if (state == kVeryFar)
    {
        ...
        ERRTNavigate2Point(robot_num, ball.position, 1, 30, &VELOCITY_PROFILE_MAMOOLI);

        Common::debug().drawCircle(ball.position, very_far_ball_dis - 90.0f, "", Common::Red);

        if (OwnRobot[robot_num].State.position.distanceTo(ball.position) < very_far_ball_dis)
        {
            state         = kFar;
            last_change_t = timer.time();
        }
    ...
    else if (state == kFar)
    {
        ...
        Common::Vec2 target_point =
            ball.position.circleAroundPoint(ball.position.angleWith(OwnRobot[robot_num].State.position), near_ball_dis);
        ERRTNavigate2Point(robot_num, target_point, 1, 20, &VELOCITY_PROFILE_AROOM);

        Common::debug().drawCircle(ball.position, far_ball_dis - 90.0f, "", Common::Pink);

        if (OwnRobot[robot_num].State.position.distanceTo(ball.position) < far_ball_dis)
        {
            hys_bank[0]++;
        }
        else
        ...
        else if (OwnRobot[robot_num].State.position.distanceTo(ball.position) > very_far_ball_dis * shmit_coeff)
        {
            state         = kVeryFar;
            last_change_t = timer.time();
        }
    ...
    else if (state == kNear)
    {
        ...
        Common::Angle toRobot    = ball.position.angleWith(OwnRobot[robot_num].State.position);
        ...
        ERRTSetObstacles(robot_num, 0, 1);
        Common::Vec2 target_point = ball.position.circleAroundPoint(newToRobot, near_ball_dis / deltaAngle.cos());
        if (near_dis_override > 0)
            ERRTNavigate2Point(robot_num, target_point, 1, 20, &VELOCITY_PROFILE_AROOM);
        else
            ERRTNavigate2Point(robot_num, target_point, 1, 20, &VELOCITY_PROFILE_MAMOOLI);

        if (OwnRobot[robot_num].State.position.distanceTo(ball.position) > far_ball_dis * shmit_coeff)
        {
            state         = kFar;
            last_change_t = timer.time();
        }
    ...
    else if (state == kKick)
    {
        ...
        if (OwnRobot[robot_num].State.position.distanceTo(ball.position) > near_ball_dis * shmit_coeff)
        {
            state         = kFar;
            last_change_t = timer.time();
        }
