...
void Ai::circleBall(int robot_num, Common::Angle tagret_angle, int shoot_pow, int chip_pow, float precision,
                     float near_dis_override)
{
    ...
    if (state == kVeryFar)
    {
        ...
        if (m_own_robot[robot_num].state().position.distanceTo(m_world_state.ball.position) < very_far_ball_dis)
        {
            state         = kFar;
            last_change_t = m_timer.time();
        }
    ...
    else if (state == kFar)
    {
        ...
        Common::Vec2 target_point = m_world_state.ball.position.circleAroundPoint(
            m_world_state.ball.position.angleWith(m_own_robot[robot_num].state().position), near_ball_dis);
        ...
        if (m_own_robot[robot_num].state().position.distanceTo(m_world_state.ball.position) < far_ball_dis)
        {
            hys_bank[0]++;
        }
        else
        ...
        else if (m_own_robot[robot_num].state().position.distanceTo(m_world_state.ball.position) >
    ...
    else if (state == kNear)
    {
        ...
        Common::Angle toRobot    = m_world_state.ball.position.angleWith(m_own_robot[robot_num].state().position);
        ...
        m_own_robot[robot_num].face(m_world_state.ball.position);
        m_own_robot[robot_num].target.angle +=
            (newToRobot + Common::Angle::fromDeg(180.0f) - m_own_robot[robot_num].target.angle) / 2.0f;
        ...
        if (m_own_robot[robot_num].state().position.distanceTo(m_world_state.ball.position) > far_ball_dis * shmit_coeff)
        {
            state         = kFar;
            last_change_t = m_timer.time();
        }
    ...
    else if (state == kKick)
    {
        ...
        m_own_robot[robot_num].target.angle = tagret_angle + Common::Angle::fromDeg(180.0f);
        navigate(robot_num, m_world_state.ball.position, VelocityProfile::aroom());
        if (shoot_pow > 0)
            m_own_robot[robot_num].shoot(shoot_pow);
        if (chip_pow > 0)
            m_own_robot[robot_num].chip(chip_pow);

        if (m_own_robot[robot_num].state().position.distanceTo(m_world_state.ball.position) > near_ball_dis * shmit_coeff)
        {
            state         = kFar;
            last_change_t = m_timer.time();
        }
