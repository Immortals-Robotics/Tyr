...
{
    Common::BallState _ball = m_state.ball;
                     ...
                     bool dribbler, bool needOppRRT)
{
    ...
    if (1) // ( std::fabs(NormalizeAngle(lastBallDirection-m_state.ball.velocity.direction) ) > 5 ) || (
           // std::fabs(lastBallMagnitude-m_state.ball.velocity.length) > 80 ) || (
           // m_state.ball.velocity.length < 100 ) || (
           // Common::Vec2::distance(m_state.ball.position, PredictedBall) < 150) )
    {
        PredictedBall = m_state.ball.position;
               ...
               // Common::Vec2::distance(m_state.ball.position, PredictedBall) < 150) )
        {
            float d = OwnRobot[robot_num].state().position.distanceTo(
                m_state.ball.position.circleAroundPoint(angle, 200.0f));
            d += m_state.ball.position.distanceTo(
                m_state.ball.position.circleAroundPoint(angle, 200.0f));
            d -= 100;
            ...
            PredictedBall.x += d * m_state.ball.velocity.x / 45.0f;
            PredictedBall.y += d * m_state.ball.velocity.y / 45.0f;
        ...
        m_state.ball.position , angle , 200.0f ) ); d += Common::Vec2::distance (
        m_state.ball.position , Common::circleAroundPoint ( m_state.ball.position , angle ,
        200.0f ) ); d -= 100;

        //float d = Common::Vec2::distance ( OwnRobot[robot_num].state().position , m_state.ball.position
        );
    ...
    Common::Vec2 ballToGoal = Common::Vec2(-side * Common::field().width, 0) - m_state.ball.position;
    ballToGoal              = ballToGoal.normalized();
    float ballVelToGoalDot =
        (m_state.ball.velocity.x * ballToGoal.x + m_state.ball.velocity.y * ballToGoal.y);
    Common::Vec2 ballVelToGoal     = Common::Vec2(ballToGoal.x * ballVelToGoalDot, ballToGoal.y * ballVelToGoalDot);
    Common::Vec2 ballVelPrepToGoal = Common::Vec2(m_state.ball.velocity.x - ballVelToGoal.x,
                                                  m_state.ball.velocity.y - ballVelToGoal.y);

    // Common::debug().draw(m_state.ball.position,m_state.ball.position+Common::Vec2(m_state.ball.velocity.x,
    // m_state.ball.velocity.y)/1.0f);
    // Common::debug().draw(m_state.ball.position,m_state.ball.position+ballVelToGoal/1.0f
    // , Pink);
    // Common::debug().draw(m_state.ball.position,m_state.ball.position+ballVelPrepToGoal/1.0f
    // , Green);
    ...
    Common::Vec2 robotToBall    = OwnRobot[robot_num].state().position - m_state.ball.position;
    ...
    if (robotToBallDot > 0.3)
        passedBall = true;
    //}
    // else if ( Common::Vec2::distance(m_state.ball.position, OwnRobot[robot_num].state().position) > 750
    // )
    else if ((robotToBallDot < -0.2) ||
             (m_state.ball.position.distanceTo(OwnRobot[robot_num].state().position) > 1050))
        passedBall = false;
    ...
    if (m_state.ball.position.distanceTo(OwnRobot[robot_num].state().position) < 400)
        circleReachedBehindBall = true;
    else if (m_state.ball.position.distanceTo(OwnRobot[robot_num].state().position) > 600)
        circleReachedBehindBall = false;

    if (0)
    {
        ERRTSetObstacles(robot_num, 0, 0);
        planner[robot_num].init(m_state.ball.position, Common::Vec2(-0, 0), 9);
        Common::Vec2 wayp = planner[robot_num].plan();

        // wayp = Common::Vec2 ( -3025 , 0 );

        angle = wayp.angleWith(m_state.ball.position);
    }
    ...
    if (dribbler)
    {
        if (OwnRobot[robot_num].state().position.distanceTo(m_state.ball.position) < 190)
        ...
        if ((kick) || (chip))
        {
            ...
            if (circleReachedBehindBall)
            {
                ...
                if (1) //! gameRestart )
                {
                    ...
                    bool el =
                        ((hehe2.deg() < 5) &&
                         (m_state.ball.position.distanceTo(OwnRobot[robot_num].state().position) < 100));
                    if (el || (elendil > 0))
                    {
    ...
    else
    {
        ...
        // if ( Common::Vec2::distance ( OwnRobot[robot_num].state().position , m_state.ball.position ) <
        // 200
        // )
