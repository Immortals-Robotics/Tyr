...
void MarkToGoalSkill::execute(Robot &t_robot)
{
    ...
    if (m_def_area_mark)
    {
        ...
        const Common::Vec2 start{State::ownGoal().x, -(penalty_area_half_width + penalty_area_dist)};
        ...
        const Common::Line              shot_line     = Common::Line::fromTwoPoints(State::ownGoal(), predictedOpp);
        const std::vector<Common::Vec2> intersections = virtual_defense_area.intersection(shot_line);

        if (intersections.size() == 2)
        {
            target = intersections[0].distanceTo(State::ownGoal()) > intersections[1].distanceTo(State::ownGoal()) ? intersections[0]
                                                                                                     : intersections[1];
        }
    ...
    {
        target = predictedOpp.pointOnConnectingLine(State::ownGoal(), m_dist);
    }
