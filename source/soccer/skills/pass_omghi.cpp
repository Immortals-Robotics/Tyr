...
void Ai::waitForOmghi(const int t_robot_num)
{
    ...
    Common::Line to_goal_line = Common::Line::fromTwoPoints(m_own_robot[t_robot_num].state().position, oppGoal());
    ...
    sBAR = ans.distanceTo(m_own_robot[t_robot_num].state().position) / sBAR;
    sBAR /= 63.0;
    // sBAR /= 10.0;
    // sBAR /= 1500000;
