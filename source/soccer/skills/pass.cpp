...
{
Common::Vec2 Ai::calculatePassPos(int t_robot_num, const Common::Vec2 &target, const Common::Vec2 &statPos, float bar)
{
...
void Ai::waitForPass(int robot_num, bool chip, const Common::Vec2 *target, Common::Vec2 *statPos)
{
    Common::Vec2 pos = calculatePassPos(robot_num, target == nullptr ? oppGoal() : *target,
                                        statPos == nullptr ? m_own_robot[robot_num].state().position : *statPos, 78);

    if (target == nullptr)
    {
        m_own_robot[robot_num].target.angle = calculateOneTouchAngle(robot_num, pos);
    }
    else
    {
        m_own_robot[robot_num].target.angle = m_own_robot[robot_num].state().position.angleWith(*target);
    }

    const OpenAngle open_angle = calculateOpenAngleToGoal(pos, robot_num);

    Common::Line shoot_line = Common::Line::fromPointAndAngle(pos, m_own_robot[robot_num].target.angle);
    ...
    if (target == nullptr)
    {
        ...
        else
        {
            ...
            m_own_robot[robot_num].shoot(40);
        }
    }
    else
    {
        if (chip)
        {
            m_own_robot[robot_num].chip(0);
        }
        else
            m_own_robot[robot_num].shoot(0);
    }
}
} // namespace Tyr::Soccer
