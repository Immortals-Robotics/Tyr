...
void Ai::GKHi(int robot_num, bool stop)
{
    ...
    Common::logDebug("GKhi: {} _ {}", ballIsGoaling(),
                     m_state.ball.position.distanceTo(OwnRobot[robot_num].state().position) /
                         m_state.ball.velocity.length());
    // side = -side;
    if (ballIsGoaling())
    {
    ...
    else
    {
        ...
        if ((obs_map.isInObstacle(m_state.ball.position)) &&
            (m_state.ball.velocity.length() < 1500) && Common::refereeState().canKickBall() && (!stop))
        {
        ...
        else
        {
			...
			Common::Vec2 ans;
			if(DIS(m_state.ball.position,Common::Vec2(side*Common::field().width,0))<2500 && m_state.ball.position.x > side*(Common::field().width -1200) ){
				ans = GK_Rail.closestPoint(m_state.ball.position);
				tmp_x = ans.getX();
				tmp_y = ans.getY();
			} else {
				tmp_x = side * penalty_x;
				tmp_y = (6.0 * m_state.ball.position.y) / 40.0f;
			}
            ...
            Common::Vec2 target = Common::Vec2(side * Common::field().width, 0)
                                      .pointOnConnectingLine(m_state.ball.position, 1500);
