...
bool Ai::position_robots(bool avoid_GK, bool avoid_DEF)
{
    ...
    for (int j = 0; j < Common::Setting::kMaxOnFieldTeamRobots; j++)
    {
        ...
        if (requiredRobots[*stm2AInum[j]] && OwnRobot[*stm2AInum[j]].State.seenState == Common::CompletelyOut)
        { // Find the robot that needs to be replaced
            ...
            for (i = 0; i < Common::Setting::kMaxOnFieldTeamRobots; i++)
            {
                if (!requiredRobots[*stm2AInum[i]] &&
                    OwnRobot[*stm2AInum[i]].State.seenState != Common::CompletelyOut && gk != *stm2AInum[i] &&
                    def != *stm2AInum[i])
                {
                    //                    std::cout<<"BEF_SWAP: "<<requiredRobots[*stm2AInum[j]]<<
                    //                    (OwnRobot[*stm2AInum[j]].State.seenState == Common::CompletelyOut)
                    //                    <<std::endl;
                    ...
                    //                    (OwnRobot[*stm2AInum[3]].State.seenState == Common::CompletelyOut)
                    //                    <<std::endl; std::cout<<"DATA_2: "<<requiredRobots[*stm2AInum[4]]<<
                    //                    (OwnRobot[*stm2AInum[4]].State.seenState == Common::CompletelyOut)
                    //                    <<std::endl;
