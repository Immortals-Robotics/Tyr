...
float Ai::calculateSwitchToAttackerScore(int robot_num)
{
    ...
    float currAttBallDis = OwnRobot[attack].state().position.distanceTo(m_state.ball.position);
    ...
    if (isDefending && marked_id != -1)
    {
        ...
        if ((m_state.opp_robot[opp].position.distanceTo(m_state.ball.position) < 400) &&
            (OwnRobot[robot_num].state().position.distanceTo(m_state.ball.position) < 400) && (currAttBallDis > 600) &&
            (m_state.ball.velocity.length() < 500))
        {
            return 0;
        }
        else
    ...
    float disToBall = OwnRobot[robot_num].state().position.distanceTo(m_state.ball.position);
