...
Common::Vec2 Ai::DefGhuz(Common::Vec2 *defendTarget)
{
    ...
    if (side == 1)
    {
        m_state.ball.position *= -1.0f;
    }

    Common::Angle alpha = Common::Vec2(-Common::field().width, 0).angleWith(m_state.ball.position);
    alpha.setDeg(std::clamp(alpha.deg(), -90.0f, 90.0f));
    int alphaSgn = Common::sign(alpha.deg());
    alpha.setDeg(std::fabs(alpha.deg()));

    float d = Common::Vec2(-Common::field().width, 0).distanceTo(m_state.ball.position);
    ...
    Common::Vec2 target = Common::Vec2(-Common::field().width, 0.0f);
