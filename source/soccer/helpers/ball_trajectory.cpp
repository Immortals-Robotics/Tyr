...
void Ai::calculateBallTrajectory()
{
    ...
    if (m_state.ball.velocity.x == 0)
        visionM.AddData(100.0f);
    else
        visionM.AddData(std::fabs(m_state.ball.velocity.y / m_state.ball.velocity.x));
    ...
    Common::debug().draw(Common::LineSegment{
        Common::Vec2(m_state.ball.position.x, ballLine.getValue(m_state.ball.position.x)),
    ...
    if (!ballLine.isAmoodi())
    {
        ...
        ballN = new_line.closestPoint(m_state.ball.position);
