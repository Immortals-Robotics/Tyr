...
void Ai::internalProcessData(Common::WorldState *worldState)
{
    ...
    if (ball.seenState != Common::CompletelyOut)
    ...
    for (int i = 0; i < Common::Setting::kMaxOnFieldTeamRobots; i++)
    {
        ...
        if (!refereeState->State || refereeState->State->get() == Common::GameState::STATE_GAME_OFF)
        {
            if (OwnRobot[i].State.OutForSubsitute)
            {
                ...
                {
                    if ((worldState->OwnRobot[j].seenState == Common::Seen) &&
                        (std::fabs(worldState->OwnRobot[j].Position.x) < field_width) &&
                        (std::fabs(worldState->OwnRobot[j].Position.y) < field_height))
                    {
                        bool suitable = true;
