...
{
    this->ball = Common::worldState().ball;
    ...
    for (int i = 0; i < Common::Setting::kMaxOnFieldTeamRobots; i++)
    {
        ...
        this->OwnRobot[i].State      = Common::worldState().own_robot[OwnRobot[i].vision_id];

        if (Common::refereeState().get() == Common::RefereeState::STATE_GAME_OFF)
        {
    ...
    for (int i = 0; i < Common::Setting::kMaxRobots; i++)
        this->OppRobot[i] = Common::worldState().opp_robot[i];
