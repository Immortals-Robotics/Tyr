...
void Ai::NormalPlayDef()
{
            ...
            if (opp == -1)
            {
                ...
                else if (own == mid1)
                {
                    if (oppAttacker != -1)
                        ...
                        ERRTNavigate2Point(own, Common::Vec2(m_state.ball.position.x, 1000), 100,
    ...
    if (goalBlocked(m_state.ball.position, 200, 90))
    {
        chip_pow = 20;
    }
    else if (!goalBlocked(m_state.ball.position, 3000, 130))
    {
    ...
    if (kicker_opp != -1)
    {
        ...
        shootAngle = m_state.opp_robot[kicker_opp].position.angleWith(m_state.ball.position);
        shoot_pow  = 1;
        chip_pow   = 0;
    }
