...
void Ai::kickoff_us_chip()
{
    ...
    ERRTNavigate2Point(mid2, Common::Vec2(Common::worldState().ball.position.x + side * 150, (Common::worldState().field.height - 300)));
    OwnRobot[mid1].face(Common::Vec2(-side * Common::worldState().field.width, 0));
    ERRTSetObstacles(mid1, true, true);
    ERRTNavigate2Point(mid1, Common::Vec2(Common::worldState().ball.position.x + side * 150, -(Common::worldState().field.height - 300)));
    ...
    if (Common::refereeState().canKickBall())
    {
        ...
        // circle_ball(attack, Common::Vec2::angleWith ( chip_target , Common::worldState().ball.position ), 100, 0, 1.0f);
