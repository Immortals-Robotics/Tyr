...
void Ai::NormalPlayAtt()
{
    ...
    receivePass(dmf, m_state.ball.position.pointOnConnectingLine(Common::Vec2(side * Common::field().width, 0), 2500));
    ...
    else
    {
        ...
        float        ballGoalDot =
            m_state.ball.velocity.normalized().dot((Common::Vec2(-side * Common::field().width, 0) - ballReachPlace).normalized());
        if (0) // ballGoalDot > -0.6 && ballGoalDot < 0.7 && m_state.ball.velocity.length > 900 )
        {
            Common::Angle passAngle = m_state.ball.velocity.toAngle();
            tech_circle(attack, passAngle, 1, 0, 1, 0, 0, 1);
        }
        else
        {
            OpenAngle openAngle = calculateOpenAngleToGoal(m_state.ball.position, attack);

            bool mid1Reached = OwnRobot[mid1].state().velocity.length() < 500;
            bool mid2Reached = OwnRobot[mid2].state().velocity.length() < 500;

            bool mid1DisOk = OwnRobot[mid1].state().position.distanceTo(m_state.ball.position) > 2000;
            bool mid2DisOk = OwnRobot[mid2].state().position.distanceTo(m_state.ball.position) > 2000;

            bool mid1PassAngleOk = (OwnRobot[mid1].state().position - m_state.ball.position)
                                       .normalized()
                                       .dot((Common::Vec2(side * Common::field().width, 0) - m_state.ball.position).normalized()) < 0.75f;

            bool mid2PassAngleOk = (OwnRobot[mid2].state().position - m_state.ball.position)
                                       .normalized()
                                       .dot((Common::Vec2(side * Common::field().width, 0) - m_state.ball.position).normalized()) < 0.75f;
            ...
            if (openAngle.magnitude.deg() < 8 &&
                (findKickerOpp(-1, 150.0f) ==
                 -1)) //&& ( m_state.ball.position.x * side < -2300 ) && ( std::fabs ( m_state.ball.position.y ) > 1800 ) )
            {

                // float passAngle = Common::Vec2::angleWith (
                // OwnRobot[randomParam<0.3?dmf:(randomParam<0.6?rmf:lmf)].state().position , m_state.ball.position );
                Common::Angle passAngle =
                    Common::Vec2(-side * 1700, Common::sign(-m_state.ball.position.y) * 1700.0f).angleWith(m_state.ball.position);
                float chip_pow = 40;

                if (mid1Suitable)
                {
                    passAngle = OwnRobot[mid1].state().position.angleWith(m_state.ball.position);
                    chip_pow  = 50.f * OwnRobot[mid1].state().position.distanceTo(m_state.ball.position) / 4000.0f;
                    chip_pow  = std::min(50.f, chip_pow);
                }
                else if (mid2Suitable)
                {
                    passAngle = OwnRobot[mid2].state().position.angleWith(m_state.ball.position);
                    chip_pow  = 50.f * OwnRobot[mid2].state().position.distanceTo(m_state.ball.position) / 4000.0f;
                    chip_pow  = std::min(50.f, chip_pow);
                }
                else
                {
                    passAngle = Common::Vec2(-side * Common::field().width, 0).angleWith(m_state.ball.position);
                    chip_pow  = 0;
                }
            ...
            else
            {
                ...
                // ADDED by Dot_Blue:
                //                float shoot_pow;
                //                if(Common::Vec2::distance(m_state.ball.position,Common::Vec2 ( -side*Common::field().width , 0 )) <
                //                2000) {
                ...
                else if (goalBlocked(m_state.ball.position, 200, 90))
                {
                    shoot_pow = 1;
                }
                else
                ...
                //	shootAngle = Common::Vec2::angleWith(m_state.ball.position, Common::Vec2(side*Common::field().width, 0));
                //	shoot_pow = 1;
                // }
    ...
    if (m_state.ball.position.y > 600)
    {
