...
void Ai::kickoff_their_one_wall()
{
    ...
    ERRTNavigate2Point(
        ...
        Common::worldState().ball.position.pointOnConnectingLine(Common::Vec2(side * Common::worldState().field.width, 0),
                                            Common::worldState().ball.position.distanceTo(Common::Vec2(side * Common::worldState().field.width, 0)) / 2.0f),
    ...
    {
        if ((std::fabs(Common::worldState().opp_robot[i].position.x) < 1500) && (std::fabs(Common::worldState().opp_robot[i].position.y) > 600) &&
    ...
    if (indexN != -1)
    {
        if (side == -1)
        {
            ...
            ERRTNavigate2Point(
                mid1, Common::worldState().opp_robot[indexN].position.pointOnConnectingLine(Common::Vec2(side * Common::worldState().field.width, 0),
        ...
        else
        {
            ...
            ERRTNavigate2Point(
                mid2, Common::worldState().opp_robot[indexN].position.pointOnConnectingLine(
                          Common::Vec2(side * Common::worldState().field.width, 0), (std::fabs(Common::worldState().opp_robot[indexN].position.x) + 14) * 1.5));
            markMap[&mid2] = indexN;
        }
    }
    else
    {
        if (side == -1)
        {
            ...
            ERRTNavigate2Point(
                mid1,
                Common::worldState().ball.position.circleAroundPoint(Common::Angle::fromDeg(20.0f) +
    ...
    if (indexP != -1)
    {
        if (side == 1)
        {
            ...
            OwnRobot[mid1].face(Common::Vec2(-side * Common::worldState().field.width, 0));
            ERRTNavigate2Point(
                mid1, Common::worldState().opp_robot[indexP].position.pointOnConnectingLine(Common::Vec2(side * Common::worldState().field.width, 0),
        ...
        else
        {
            ...
            OwnRobot[mid2].face(Common::Vec2(-side * Common::worldState().field.width, 0));
            ERRTNavigate2Point(
                mid2, Common::worldState().opp_robot[indexP].position.pointOnConnectingLine(Common::Vec2(side * Common::worldState().field.width, 0),
                                                                      (std::fabs(Common::worldState().opp_robot[indexP].position.x) + 14) * 1.5));
            markMap[&mid2] = indexP;
        }
    }
        ...
        if (side == 1)
        {
            ...
            ERRTNavigate2Point(
                ...
                Common::worldState().ball.position.circleAroundPoint(
                    Common::Angle::fromDeg(20) + Common::worldState().ball.position.angleWith(Common::Vec2(side * Common::worldState().field.width, 0)), 790),
                0, 100);
            markMap[&mid1] = -1;
        }
        else
