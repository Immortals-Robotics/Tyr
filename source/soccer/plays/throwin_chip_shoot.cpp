...
void Ai::throwin_chip_shoot()
{
    ...
    if (timer.time() > 4)
    {
        ...
        circle_ball(dmf,
                    Common::Vec2(-side * Common::worldState().field.width, Common::sign(ball.position.y) * 200.0f).angleWith(ball.position),
                    0, 15, 1.0f);
    }
    else
    {
        // tech_circle(dmf,Common::Vec2::angleWith ( Common::Vec2 ( -side*2995 , 0 ) , ball.position ) ,0,0,0,1);
        circle_ball(dmf, Common::Vec2(-side * Common::worldState().field.width, 0).angleWith(ball.position), 0, 0, 1.0f);
    }
    ...
    if (randomParam < 0.0)
        ERRTNavigate2Point(
            attack, ball.position.pointOnConnectingLine(Common::Vec2(-side * Common::worldState().field.width, 0), 350 + 700 * reached));
    else if (randomParam < 0.5)
        ERRTNavigate2Point(attack, ball.position.pointOnConnectingLine(
                                       Common::Vec2(-side * Common::worldState().field.width, Common::sign(-ball.position.x) * 2000.0f),
                                       350 + 700 * reached));
    else
        ERRTNavigate2Point(attack, ball.position.pointOnConnectingLine(
                                       Common::Vec2(-side * Common::worldState().field.width, Common::sign(ball.position.x) * 2000.0f),
