...
void Ai::throwin_chip_shoot()
{
    ...
    ERRTNavigate2Point(mid2, Common::Vec2(-side * 1500, Common::sign(m_state.ball.position.y) * 2500.0f),
    ...
    ERRTNavigate2Point(mid1, Common::Vec2(-side * 3500, Common::sign(m_state.ball.position.y) * 2500.0f),
                       70, VelocityProfile::Type::Mamooli);
    ...
    if (timer.time() > 4)
    {
        ...
        // m_state.ball.position ) ,0,30,0,1);
        circle_ball(dmf,
                    Common::Vec2(-side * Common::field().width,
                                 Common::sign(m_state.ball.position.y) * 200.0f)
    ...
    {
        // tech_circle(dmf,Common::Vec2::angleWith ( Common::Vec2 ( -side*2995 , 0 ) ,
        // m_state.ball.position ) ,0,0,0,1);
        circle_ball(
            dmf,
            Common::Vec2(-side * Common::field().width, 0).angleWith(m_state.ball.position), 0,
            0, 1.0f);
    }
    ...
    obs_map.addCircle({m_state.ball.position, 320.0f});
    if (randomParam < 0.0)
        ERRTNavigate2Point(attack, m_state.ball.position.pointOnConnectingLine(
                                       Common::Vec2(-side * Common::field().width, 0), 350));
    else if (randomParam < 0.5)
        ERRTNavigate2Point(attack, m_state.ball.position.pointOnConnectingLine(
                                       Common::Vec2(-side * Common::field().width,
                                                    Common::sign(-m_state.ball.position.x) * 2000.0f),
                                       350));
    else
        ERRTNavigate2Point(attack, m_state.ball.position.pointOnConnectingLine(
                                       Common::Vec2(-side * Common::field().width,
