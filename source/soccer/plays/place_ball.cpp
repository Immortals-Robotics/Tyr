...
void Ai::our_place_ball_shoot()
{
    ...
    if (m_state.ball.position.distanceTo(Common::refereeState().place_ball_target) > 100)
    {
        move_angle   = Common::refereeState().place_ball_target.angleWith(m_state.ball.position);
    ...
    ERRTNavigate2Point ( mid1 , PointOnConnectingLine(m_state.ball.position,
    Common::Vec2(side*Common::field().width, 0), DIS(m_state.ball.position,
    ...
    ERRTNavigate2Point(
        ...
        Common::Vec2(0, -100) +
            m_state.ball.position.pointOnConnectingLine(
                ...
                m_state.ball.position.distanceTo(Common::Vec2(side * Common::field().width, 0)) / 3.0f),
    ...
    ERRTNavigate2Point(
        ...
        m_state.ball.position.circleAroundPoint(
            m_state.ball.position.angleWith(Common::Vec2(side * Common::field().width, 0)), 1090),
        100, VelocityProfile::Type::Aroom);
    ...
    if (FUNC_state == 0)
    {
        if (m_state.ball.position.distanceTo(Common::refereeState().place_ball_target) < 95)
        {
            FUNC_state = 7;
            FUNC_CNT   = 0;
            t_ang.setDeg(0.0f);
        ...
        else if (m_state.ball.position.distanceTo(Common::refereeState().place_ball_target) < 1000)
        {
            FUNC_state = 3;
            FUNC_CNT   = 0;
        }
        ...
        //        obs_map.addCircle({m_state.ball.position, 150.0f});
        //        ERRTNavigate2Point(attack, CircleAroundPoint(m_state.ball.position, move_angle, 250), 0,
    ...
    else if (FUNC_state == 1)
    {
        ...
        if (OwnRobot[attack].state().position.distanceTo(m_state.ball.position) > 400)
        {
            FUNC_CNT++;
        }
        if (FUNC_CNT >= 20)
    ...
    else if (FUNC_state == 3)
    {
        ...
        ERRTSetObstacles(attack, 0, 0);
        obs_map.addCircle({m_state.ball.position, 150.0f});
        ERRTNavigate2Point(attack, m_state.ball.position.circleAroundPoint(temp_opp_ang, 250), 40,
                           VelocityProfile::Type::Aroom);
        ERRTSetObstacles(dmf, 0, 0);
        obs_map.addCircle({m_state.ball.position, 150.0f});

        ERRTNavigate2Point(dmf, m_state.ball.position.circleAroundPoint(move_angle, 250), 40,
                           VelocityProfile::Type::Aroom);
        ...
        if (m_state
            ...
            m_state.ball.velocity.length() < 10)
        {
    ...
    else if (FUNC_state == 4)
    {
        ...
        bool ball_has_slipt = ((OwnRobot[attack].state().position + OwnRobot[dmf].state().position) / 2)
                                  .distanceTo(m_state.ball.position) > 300;

        if (m_state.ball.seen_state == Common::SeenState::Seen && ball_has_slipt)
        {
            FUNC_state = 3;
            FUNC_CNT   = 0;
        }
    ...
    else if (FUNC_state == 6)
    {
        ...
        Navigate2Point(dmf, Common::refereeState().place_ball_target.circleAroundPoint(t_ang, 250), 20,
                       VelocityProfile::Type::BallPlaceKheyliSooski);
    ...
    else
    {
        ...
        OwnRobot[attack].face(m_state.ball.position); // TODO test this
        ...
        //        Navigate2Point(attack, CircleAroundPoint(m_state.ball.position, t_ang, 170), 0, 20,
        //        VelocityProfile::Type::BallPlaceKheyliSooski); t_ang = NormalizeAngle(OwnRobot[dmf].state().angle +
        //        180); Navigate2Point(dmf, CircleAroundPoint(m_state.ball.position, t_ang, 170),  20,
        //        VelocityProfile::Type::BallPlaceKheyliSooski);

        bool success = Common::refereeState().place_ball_target.distanceTo(m_state.ball.position) < 100.0;

        Common::logDebug("______IN___STATE_DONE_____");
        Common::logDebug("{}", Common::refereeState().place_ball_target - m_state.ball.position);
        if (success)
        {
    ...
    Common::logDebug("___DIS___{}",
                     Common::refereeState().place_ball_target.distanceTo(m_state.ball.position));
}
...
void Ai::our_place_ball_shoot_V2()
{
            ...
            if (own == mid1)
            {
                ...
                ERRTNavigate2Point(
                    ...
                    PointOnConnectingLine(
                        m_state.ball.position, Common::Vec2(side * Common::field().width, 0),
                        DIS(m_state.ball.position, Common::Vec2(side * Common::field().width, 0)) / 3.0f),
                    100, VelocityProfile::Type::Aroom);
            }
            else if (own == mid2)
            {
                ...
                ERRTNavigate2Point(
                    ...
                    CircleAroundPoint(m_state.ball.position,
                                      NormalizeAngle(AngleWith(m_state.ball.position,
                                                               Common::Vec2(side * Common::field().width, 0))),
                                      1090),
                    100, VelocityProfile::Type::Aroom);
            }
    ...
    if (FUNC_state == -2)
    {
        ...
        if (last_state_ball_pos.distanceTo(m_state.ball.position) > 400)
        {
            FUNC_CNT++;
        }
        if (FUNC_CNT >= 20)
    ...
    else if (FUNC_state == -1)
    {
        ...
        {
            if (m_state.ball.position.x > 0)
            {
                if (m_state.ball.position.y > 0)
                {
                    if (m_state.ball.position.x > Common::field().width)
                        outFieldAng.setDeg(155.0f);
                    else // So std::fabs(m_state.ball.position.y) > Common::field().height
            ...
            else
            {
                if (m_state.ball.position.y > 0)
                {
                    if (std::fabs(m_state.ball.position.x) > Common::field().width)
        ...
        if (FUNC_CNT >= 10)
        {
            FUNC_state          = -2;
            FUNC_CNT            = 0;
            last_state_ball_pos = m_state.ball.position;
        }
    }
    else if (FUNC_state == 0)
    {
        ...
        else if (m_state.ball.position.distanceTo(Common::refereeState().place_ball_target) < 10000.0f)
        {
            FUNC_state = 3;
            FUNC_CNT   = 0;
        }
        else if (outOfField(m_state.ball.position))
    ...
    Common::logInfo("___DIS___{}",
                    Common::refereeState().place_ball_target.distanceTo(m_state.ball.position));
    Common::logInfo("___BALL__POS_XY__{}, {}", m_state.ball.position.x,
                    m_state.ball.position.y);
    Common::logInfo("___TARGET__POS_XY__{}, {}", Common::refereeState().place_ball_target.x,
                    Common::refereeState().place_ball_target.y);
    Common::logInfo("__OUT__{}", outOfField(m_state.ball.position));
}
...
void Ai::our_place_ball_shoot_taki()
{
    ...
    if (m_state.ball.position.distanceTo(Common::refereeState().place_ball_target) > 150)
    {
        move_angle = Common::refereeState().place_ball_target.angleWith(m_state.ball.position);
    ...
    else if (FUNC_state == 3)
    {
        ...
        //        obs_map.addCircle({m_state.ball.position, 150.0f});
        float max_spd = 10.0f;
        if (m_state.ball.position.distanceTo(Common::refereeState().place_ball_target) < 350)
            max_spd = 3.0f;
