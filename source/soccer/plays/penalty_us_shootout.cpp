...
void Ai::penalty_us_shootout()
{
    ...
    else if (Common::distance(ball.Position, Common::vec2(-side * field_width, 0)) > 3000)
    {
        circle_ball(attack, Common::angle_with(Common::vec2(-side * field_width, 0), ball.Position), 1, 0, 0);
        LOG_INFO("step1 - Moving forward - waiting to get close to the opp goal");
    }
    else
    {
        circle_ball(attack, Common::angle_with(Common::vec2(-side * field_width, 400), ball.Position), 60, 0, 0);
