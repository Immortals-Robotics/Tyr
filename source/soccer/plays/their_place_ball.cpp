    ...
    for (std::map<int *, int>::const_iterator i = markMap.begin(); i != markMap.end(); ++i)
    {
        ...
        if (opp == -1)
        {
            ...
            else if (own == mid2)
            {
                ...
                navigate(mid2,
                         CircleAroundPoint(m_world_state.ball.position,
                                           NormalizeAngle(-20 + AngleWith(m_world_state.ball.position, ownGoal())),
                                           650),
                         VelocityProfile::aroom());
            ...
            else if (own == mid1)
            {
                ...
                navigate(mid1,
                         CircleAroundPoint(m_world_state.ball.position,
                                           NormalizeAngle(20 + AngleWith(m_world_state.ball.position, ownGoal())), 650),
                         VelocityProfile::aroom());
            ...
            else if (own == attack)
            {
                ...
                navigate(attack,
                         CircleAroundPoint(m_world_state.ball.position,
                                           NormalizeAngle(AngleWith(m_world_state.ball.position, ownGoal())), 650),
                         VelocityProfile::aroom());
