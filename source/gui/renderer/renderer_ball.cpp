...
void Renderer::drawBall(const Protos::SSL_DetectionBall &ball, bool draw_goal_lines)
{
    ...
    if (draw_goal_lines)
    {
        Common::Vec2 ourgoal_p1 = Common::Vec2(Common::worldState().field.width / -2, Common::worldState().field.goal_width / 2);
        Common::Vec2 ourgoal_p2 = Common::Vec2(Common::worldState().field.width / -2, Common::worldState().field.goal_width / -2);
