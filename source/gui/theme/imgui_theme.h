...
struct ImGuiThemeTweaks
{
    ...
    float Rounding = -1.f;
    // If rounding is applied, scrollbar rounding needs to be adjusted to be visually pleasing in conjunction with other
    // widgets roundings. Only applied if Rounding > 0.f)
    ...
    // HSV Color tweaks
    //
    // Change the hue of all widgets (gray widgets will remain gray, since their saturation is zero). If < 0, this is
    // ignored.
    float Hue = -1.f;
    // Multiply the saturation of all widgets (gray widgets will remain gray, since their saturation is zero). If < 0,
    // this is ignored.
...
// Show the theme selection listbox, the theme tweak widgets, as well as ImGui::ShowStyleEditor. Returns true if
// modified (Warning, when using ShowStyleEditor, no info about modification is transmitted)
