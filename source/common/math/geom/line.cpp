...
{
Line::Line(const float t_a, const float t_b, const float t_c)
    : a(t_a), b(t_b), c(t_c)
{}
...
std::vector<Vec2> Line::intersect(const Circle &t_circle) const
{
    ...
    {
        const auto sol = abcFormula(
            1, -2 * t_circle.center.y,
            ((-c / b) - t_circle.center.x) * ((-c / b) - t_circle.center.x) + t_circle.center.y * t_circle.center.y -
            t_circle.r * t_circle.r);
    ...
    // circle: (x-t_circle.center.x)^2 + (y-t_circle.center.y)^2 = r^2, with t_circle.center.x = center.x and t_circle.center.y = center.y
    // fill in:x^2 -2hx + t_circle.center.x^2 + (da*x-db)^2 -2k(da*x-db) + t_circle.center.y^2 - r^2 = 0
    //         x^2 -2hx + t_circle.center.x^2 + da^2*x^2 + 2da*db*x + db^2 -2k*da*x -2k*db
    //                                                         + t_circle.center.y^2 - r^2 = 0
    //       (1+da^2)*x^2 + 2(da*db-t_circle.center.x-t_circle.center.y*da)*x + h2 + db^2  -2k*db + t_circle.center.y^2 - r^2 = 0
    ...
    const float d_c = t_circle.center.x * t_circle.center.x + db * db - 2 * t_circle.center.y * db + t_circle.center.y
                       *
                       t_circle.center.y - t_circle.r * t_circle.r;
